...
funcConnCalc <- function(filePattern, condition,starttick,endtick){
  ...
  transitionTable <- left_join(agentStartPatch,agentEndPatch,by = c('agent_id','seed')) %>% select(tickStart = tick.x,
                                                                                ...
                                                                                startPatch,
                                                                                endPatch)
  ata <- transitionTable %>% group_by(seed,.drop=FALSE) %>% filter(startPatch == 'A' & endPatch =='A') %>%  summarise(ata_num = n())
  
  atb <- transitionTable %>% group_by(seed,.drop=FALSE) %>% filter(startPatch == 'A' & endPatch =='B') %>%  summarise(atb_num = n())
  
  atc <- transitionTable %>% group_by(seed,.drop=FALSE) %>% filter(startPatch == 'A' & endPatch =='C') %>%  summarise(atc_num = n())
